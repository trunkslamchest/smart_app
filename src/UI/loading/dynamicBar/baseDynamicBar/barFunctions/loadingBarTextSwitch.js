export var loadingBarTextSwitch = function(barType, status) {
  ...
  if(barType === 'authLogIn'){
    ...
    if(status === 'getAchievements') barText = 'Fetching SmartApp™ Achievements'
    if(status === 'storeAchievementsSuccess') barText = 'SmartApp™ Achievements Stored'
  ...
  if(barType === 'refresh'){
    if(status === 'authGoogle') barText = 'authGoogle'
    if(status === 'authUser') barText = 'authUser'
    if(status === 'storeUserInfo') barText = 'storeUserInfo'
    if(status === 'storeUserQuestions') barText = 'storeUserQuestions'
    if(status === 'storeAchievements') barText = 'storeAchievements'
    if(status === 'storeQuestionTotals') barText = 'storeQuestionTotals'
    if(status === 'authSuccess') barText = 'authSuccess'
    if(status === 'authValid') barText = 'authValid'
