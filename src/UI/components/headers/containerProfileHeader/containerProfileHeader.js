...
const ContainerProfileHeader = (props) => {
  ...
  return(
    <div className='container_profile_header'>
      <div className='container_profile_header_left'>
      { !!props.avatar &&
       <img
          alt={ `${props.user_name}'s Avatar` }
          className='container_profile_header_left_img'
          src={ props.avatar }
          title={ `${props.user_name}'s Avatar` }
        />
      }
        <div className='container_profile_header_left_text'>
          <div className='container_profile_header_left_text_top'>
            <h3>{ props.user_name }</h3>
            { !!props.email && <h5>{ props.email }</h5> }
            { (!!props.first_name || !!props.last_name) &&
              <div className='container_profile_header_left_text_sub_countainer'>
                { !!props.first_name && <span>{ props.first_name }</span> }
                { !!props.last_name && <span>{ props.last_name }</span> }
              </div>
            }
          </div>
          <div className='container_profile_header_left_text_bottom'>
      ...
      <div className='container_profile_header_right'>
        { !!props.buttons &&
